...
class   variogram_2:
	...
	Azimuth = float(self.params['Azimuth']['value'])
	Dip = float(self.params['Dip_2']['value'])
	n_directions = float(self.params['n_directions']['value'])
	...
	if ((cdimensaoxt == cdimensaoxh) and (cdimensaoyt == cdimensaoyh) and (cdimensaozt == cdimensaozh)):
		...
		azimute = []
		azimute = np.linspace(0+Azimuth,360+Azimuth,n_directions)
		...
		for z in azimute:
			...
			if (Dip != 90 and Dip != 180):
				dipatua = math.degrees(math.atan(math.tan(math.radians(Dip))*math.sin(math.radians(90-diferenca))))
			else:
				dipatua = 90
			dip.append(dipatua)
		...
		indice_a = range(0,len(azimute))
		indice_b = range(0,len(dip))
		# Calculate admissible pairs 
		for a, d in zip(indice_a,indice_b): 
			azm = azimute[a]
			dipv = dip[d]
		...
		if (C_PairWise == 1): 		
			...
			for i in range(0,len(v_valores_admissiveis_h)):
				...
				dgmedio = 0
				for j in range(0, len(flutuanteh)):
					if (flutuanteh[j] + flutuantet[j] > 0):
		...
		for i in range(0, len(azimutes)):
			...
			direction1 = math.cos(math.radians(Dip[0]))*math.sin(math.radians(azim[0]))
			direction2 = math.cos(math.radians(Dip[0]))*math.cos(math.radians(azim[0]))
